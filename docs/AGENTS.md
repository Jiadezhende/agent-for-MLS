...
1. **PlannerAgent** — one LLM call with forced tool `assign_workers`; system prompt built dynamically from `AgentDefinition.description` of all registered types. Falls back to 1:1 on failure.
...
| `description` | Capability + routing/grouping rules injected into Planner system prompt |
