...
Step 1: Get the Roofline. Have the Agent read sm__throughput and gpu__compute_memory_throughput first.
Step 2: Characterize.
If Memory % > Compute %, dive into dram and l1/l2 metrics.
If Compute % > Memory %, check if tensor_op is being triggered.
Step 3: Look for Anomalies. Check if Occupancy is too low or if Bank Conflict exists.
Step 4: Map back to Code. Relate these metrics to your CUDA kernel code (e.g., is the loop unrolling insufficient? Is a __shared__ array missing?).
...
Memory Latency Hierarchy: Measure the exact access cycles for L1 Cache, L2 Cache, and DRAM. This requires the Agent to generate “Pointer Chasing” kernels to bypass hardware prefetchers.
Effective Peak Bandwidth: Determine the maximum achievable throughput for Shared Memory and Global Memory (VRAM) under current conditions.
L2 Cache Capacity: Identify the “cliff” in the latency-vs-size curve to pinpoint the exact physical size of the L2 cache.
Actual Boost Frequency: Report the stable core clock frequency (MHz) while the GPU is under sustained compute load.
Resource Penalties: Quantify the latency cost of a bank conflict in Shared Memory compared to a conflict-free access.
...
Non-Standard Frequency Locking: The GPU core and memory clocks may be locked at arbitrary, non-standard frequencies (e.g., 825 MHz instead of 1410 MHz) using nvidia-smi. A static lookup table will provide incorrect bandwidth/GFLOPS results.
Resource Masking (SM Limiting): The system may restrict the kernel execution to a subset of SMs or limit the available memory per block via CUDA environment variables.
Instruction Set Restrictions: Standard API calls like cudaGetDeviceProperties may be intercepted or report virtualized/misleading data.
...
Student Agent Output: The final results.json containing the identified hardware metrics and the reasoning/logs provided by your Agent.
Ground Truth Data: The exact hardware parameters measured by our reference benchmarks under the specific environment (including any active frequency locks or resource masking).
Experimental Evidence: Summaries of the ncu traces and micro-benchmarks generated during the Agent’s execution.
Scoring Rubric (100 Points Total):

Numerical Alignment (70 Points): The LLM will compare your Agent’s reported values against the Ground Truth.
...
Engineering Reasoning & Methodology (30 Points): The LLM will evaluate the “how” and “why” behind your Agent’s conclusions.
