...
class TestDetectArchFlags:
    def test_returns_arch_flag_from_compute_cap(self):
        from executor import _detect_arch_flags
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("12.0\n")):
            result = _detect_arch_flags()
        assert result == "-arch=sm_120"

    def test_returns_none_on_nvidia_smi_failure(self):
        from executor import _detect_arch_flags
        with patch("executor._run_subprocess", return_value=_mock_subprocess_fail()):
            result = _detect_arch_flags()
        assert result is None

    def test_returns_none_on_non_numeric_output(self):
        from executor import _detect_arch_flags
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("N/A\n")):
            result = _detect_arch_flags()
        assert result is None

    def test_strips_dot_correctly(self):
        from executor import _detect_arch_flags
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("8.6\n")):
            result = _detect_arch_flags()
...
class TestAutodetectEnvArch:
    def test_adds_arch_flag_when_not_set(self):
        from executor import _autodetect_env
        cfg = _make_cfg()
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("12.0\n")):
...
class TestAutodetectEnvCcbin:
    ...
    def test_detects_msvc_on_windows(self, tmp_path):
        from executor import _autodetect_env, _detect_msvc_ccbin
        ...
        cfg = _make_cfg()
        with patch("executor._run_subprocess", side_effect=fake_subprocess), \
...
class TestAutodetectEnvToolPaths:
    ...
    def test_ncu_detected_from_search_path(self, tmp_path):
        ...
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("12.0\n")), \
             ...
             patch("executor._NCU_SEARCH_PATHS_WIN", [fake_ncu]):
            updated, notes = _autodetect_env(cfg)
    ...
    def test_nsys_detected_from_search_path(self, tmp_path):
        ...
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("12.0\n")), \
             patch("shutil.which", return_value=None), \
             patch("executor._NSYS_SEARCH_PATHS_WIN", [fake_nsys]):
            updated, notes = _autodetect_env(cfg)
    ...
    def test_no_override_when_tool_not_found_anywhere(self):
        ...
        with patch("executor._run_subprocess", return_value=_mock_subprocess_ok("12.0\n")), \
             patch("shutil.which", return_value=None), \
             patch("executor._NCU_SEARCH_PATHS_WIN", []), \
             patch("executor._NSYS_SEARCH_PATHS_WIN", []):
            updated, notes = _autodetect_env(cfg)
